...
Image[] enemy_skin = new Image[33];
Image[] enemy_skin2 = new Image[33];
Image[] enemy_skin3 = new Image[33];
Image[] player_skin = new Image[33];
Image back = new Image();
Image pla = new Image();
Image pra = new Image();
Image ela = new Image();
Image era = new Image();
Image[] lock = new Image[2];
...
boolean[] lockedskin = new boolean[33];
...
void setup() {
 ...
 pla.setImage ("left arrow.png");
 pra.setImage ("right arrow.png");
 ela.setImage ("left arrow.png");
 era.setImage ("right arrow.png");
 ...
 for(i=0;i<enemy_skin.length;i++) {
   ...
   enemy_skin2[i]=new Image();
   enemy_skin2[i].setImage("skin (" + (i+1) + ").png");
   enemy_skin3[i]=new Image();
   enemy_skin3[i].setImage("skin (" + (i+1) + ").png");
 }
    //setup player skin//
...
void draw() {
 ...
 pl.penThickness=x;
  lock[0].x=player_skin[currentplayerskin].x;
   lock[0].y=player_skin[currentplayerskin].y;
     lock[1].x=enemy_skin[currentenemyskin].x;
     lock[1].y=enemy_skin[currentenemyskin].y;
 ...
 if(bgtime>=1000 && bgint<5){
  ...
  enemy_skin[currentenemyskin].x =en[0].x -70;
  enemy_skin[currentenemyskin].y = en[0].y -70;
  enemy_skin2[currentenemyskin].x =en[1].x -70;
  enemy_skin2[currentenemyskin].y = en[1].y -70;
    enemy_skin3[currentenemyskin].x =en[2].x -70;
  enemy_skin3[currentenemyskin].y = en[2].y -70;
...
if (diflevelint==2){
  enemy_skin[currentenemyskin].draw(); 
  enemy_skin2[currentenemyskin].draw();
}
if (diflevelint==3){
  enemy_skin[currentenemyskin].draw();
  enemy_skin2[currentenemyskin].draw(); 
  enemy_skin3[currentenemyskin].draw();
}
...
if (lockedskin[currentplayerskin]==true){
  lock[0].draw();
}
if (lockedskin[currentenemyskin]==true){
  lock[1].draw();
}
if (lockedskin[currentplayerskin]==false){
  player_skin[currentplayerskin].draw();
}
if (lockedskin[currentenemyskin]==false){
  enemy_skin[currentenemyskin].draw();
}
if (shopflag==true){
  pla.draw();
...
void mouseClicked (){
  if(play.pointInShape(mouseX,mouseY)) {
    if (lockedskin[currentplayerskin]==false &&lockedskin[currentenemyskin]==false){
    shopflag=false;
        if(menuflag==true){
      click.play();
    }
   menuflag=false; 
    pl.draw();}
    if (lockedskin[currentplayerskin]==true ||lockedskin[currentenemyskin]==true){
  }
  }
  ...
  if(shop.pointInShape(mouseX,mouseY)){
   ...
   menuflag=false;
   back.draw();
   if(currentplayerskin != 33 ){
     pra.draw();
   }
   if(currentplayerskin != 1 ){
   pla.draw();
  }
  if(currentenemyskin != 33 ){
   era.draw();
  }
  if(currentenemyskin != 1){
  ela.draw();
  } 
  }
 if(back.pointInShape(mouseX,mouseY)){
    if (diflevelflag==true){
      menuflag=true;
      diflevelflag=false;
    }
...
if (pla.pointInShape(mouseX,mouseY) && currentplayerskin != 1){
  if (shopflag==true){
    click.play();
    currentplayerskin = currentplayerskin - 1;
    }
  player_skin[currentplayerskin].x =currentplayerskintext.x +120;
...
if (pra.pointInShape(mouseX,mouseY) && currentplayerskin != 33){
  if (shopflag==true){
    click.play();
    currentplayerskin = currentplayerskin + 1;
    }
...
if (ela.pointInShape(mouseX,mouseY) && currentenemyskin != 1){
  if (shopflag==true){
  click.play();

  currentenemyskin=currentenemyskin-1;
...
if (era.pointInShape(mouseX,mouseY)){
  if (shopflag==true){
...
if (player_skin[currentplayerskin].pointInShape(mouseX,mouseY) && moneyint>=100&&shopflag==true&&lockedskin[currentplayerskin]==true){
...
shopflag=true;
player_skin[currentplayerskin].draw();
  lockedskin[currentplayerskin]=false;
}
if (enemy_skin[currentenemyskin].pointInShape(mouseX,mouseY) && moneyint>=100 && shopflag==true&&lockedskin[currentenemyskin]==true){
  moneyint=moneyint-100;
  menuflag=false;
menuflag=true;
shopflag=false;
shopflag=true;
  lockedskin[currentenemyskin]=false;
  enemy_skin[currentenemyskin].draw();
